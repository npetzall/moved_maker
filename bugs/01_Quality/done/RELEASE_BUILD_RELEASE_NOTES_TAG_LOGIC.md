...
├── pyproject.toml          # Project metadata and dependencies (uv project)
├── uv.lock                 # Dependency lock file (generated by uv)
├── src/
│   └── release_notes/
│       ├── __init__.py
│       ├── __main__.py     # Entry point
│       ├── git.py          # Git operations (tags, commits)
│       ├── parser.py        # Conventional Commits parsing
...
- Structured sections with emoji indicators (✨ Features, 🐛 Bug Fixes, 💥 Breaking Changes, etc.)
...
✅ **COMPLETED** - Implementation complete, all tests passing, ready for CI testing
...
2. **Implement commit formatting**
   ...
   - [x] Implement `format_section_header(type, emoji)` function
     ...
     - Map types to appropriate emojis (✨ feat, 🐛 fix, 💥 breaking, etc.)
