...
pub fn build_from_expression(resource_type: &str, resource_name: &str) -> Expression {
    ...
    let body = parse_body(&attr_str).unwrap_or_else(|_| {
...
"#).unwrap();
        ...
        assert!(blocks.iter().all(|b| b.ident.value().to_string() == "resource"));
    }
...
"#).unwrap();
    ...
    fn test_filename_extraction() {
        ...
        assert!(block.decor().prefix().unwrap_or(&"".into()).contains("From: test.tf"));
    }

    #[test]
    #[should_panic(expected = "path must have filename")]
    fn test_filename_extraction_panic() {
        // This is hard to test directly, but the expect message is clear
        let path = Path::new("/");
        let _ = path.file_name().expect("path must have filename");
    }
...
"#).unwrap();
    ...
    #[test]
    fn test_resource_with_count_meta_argument() {
        let temp_dir = TempDir::new().unwrap();
        let file = temp_dir.path().join("main.tf");
        fs::write(&file, r#"
resource "aws_instance" "web" {
...
"#).unwrap();
    ...
    #[test]
    fn test_resource_with_for_each_meta_argument() {
        let temp_dir = TempDir::new().unwrap();
        let file = temp_dir.path().join("main.tf");
        fs::write(&file, r#"
resource "aws_instance" "web" {
